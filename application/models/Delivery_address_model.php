...
class Delivery_address_model extends MY_Model
{
	...
	{
		parent::__construct("delivery_address");
        $this->tables['name'] = "delivery_address";
	}
	/**
	 * create
	 *
	...
	public function delete( $data_param  )
    {
		if( array_key_exists( "id", $data_param ) ){
			...
			foreach( $this->delivery_addresses( )->result() as $item )
			{
	 ...
	 * @author madukubah
	 */
	public function delivery_address( $id = NULL  )
    {
		if (isset($id))
		{
	 ...
	 * @author Ben Edmunds
	 */
	public function last_delivery_address( $company_id = NULL )
	{

		$this->select( $this->tables['name'].".id" );
		$this->select( $this->tables['name'].".code" );

		$this->limit(1);
		$this->order_by($this->tables['name'].'.id', 'desc');

		$this->delivery_addresses( $company_id );
	 ...
	 * @author madukubah
	 */
    public function delivery_addresses( $company_id = NULL )
    {
		...
		if (isset($company_id))
		{
			...
			if (isset($company_id) && !empty($company_id))
			{
				...
				$this->db->join(
				    "company",
				    "company.id = ".$this->tables['name'].'.company_id',
				    'inner'
				);
			}
			
			$this->db->where_in( "company.id" ,  ( $company_id ) ) ;
		}
